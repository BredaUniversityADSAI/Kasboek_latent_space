...
def normalize_by_eye(iris_center, eye_bbox_wh):
    ...
    # express iris center relative to eye box center in normalized coords [-0.5..0.5]
    cx = (iris_center[0] - (x_min + w/2.0)) / w
    cy = (iris_center[1] - (y_min + h/2.0)) / h
    return np.array([cx, cy], dtype=np.float32)


# direction classification with thresholds (allow diagonal)
def classify_gaze(norm_offset, horiz_thr, vert_thr, deadzone):
    x, y = norm_offset
    # if within deadzone for both axes -> center
...
class EyeTracker:
    def __init__(self, flip=True):
        ...
        self.flip = flip
        # rolling buffers for median smoothing
    ...
    def process_frame(self, frame, only_compute=False):
        ...
        if results.multi_face_landmarks:
            ...
            # eye exterior corners to determine inter-ocular width (scale invariant measure)
            ...
            # store raw normalized vector for calibration uses
            ...
            # compute inter-ocular distance for scale metrics (use corner centers)
